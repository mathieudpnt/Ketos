...
class ActiveLearningBatchGenerator2():
    ...
    def __init__(self, table, batch_size, instance_function=None, shuffle=False, refresh=False, return_indeces=False,\
                    max_keep=0, conf_cut=0, seed=None, batch_norm=False, verbosity=0):
        ...
        self.return_indeces = return_indeces
        self.max_keep = max_keep
        self.conf_cut = conf_cut
        ...
        self.poor_indeces = np.empty(dtype=int)
    ...
    def __get_batch_indices__(self):
        ...
        """
        # number of examples kept from previous batch
        num_keep = int(min(self.poor_indeces.shape[0], self.max_keep * self.batch_size)

        # number of new examples
        num_new = self.batch_size - num_keep

        # select randomly from poorly predicted examples in previous batch
        new_batch = np.random.choice(self.poor_indeces, num_keep, replace=False)

        # select new examples
        i1 = self.batch_indices[-1] + 1
        i2 = min(i1 + num_new, self.data_size)
        new_batch = self.indeces[i1:i2]

        # if necessary, go back to beginning to complete batch
        dn = self.batch_size - new_batch.shape[0]
        if dn > 0:
            new_batch = np.concatenate((new_batch, self.indeces[:dn]))

        # refresh at end of epoch
        epoch_end = (i2 == self.data_size or dn > 0)
        if self.refresh and epoch_end:
            self.indices = self.__refresh_indices__()

        # shuffle within batch
        np.random.shuffle(new_batch)
    ...
    def __next__(self):
        ...
        self.batch_indeces = self.__get_batch_indices__()

        X = self.data[self.batch_indeces]['data']
        Y = self.data[self.batch_indeces]['label']
        ...
        if self.return_indeces:
            return (self.batch_indeces, X, Y)
        else:
            return (X, Y)
    ...
    def performance_on_batch(self, predictions, confidences):
        ...
        assert len(predictions) == self.batch_size, 'length of prediction and confidence arrays do not match the number of samples drawn in the last iteration'
        ...
        self.poor_indeces = self.batch_indeces[poor]
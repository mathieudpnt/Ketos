...
def standardize_annotations(path, sep=',', mapper=None, signal_labels=None, backgr_labels=[]):
    """ Standardize the annotation table format.
        ...
        Args:
            path: str
                Full path to input annotation table. Expects csv format.
            sep: str
    ...
    # read input file
    df = pd.read_csv(path, sep=sep, dtype = {'label': str})
    ...
    drop_cols = [x for x in df.columns.values if x not in keep_cols]
    df = df.drop(drop_cols, axis=1)

    # check that dataframe has minimum required columns
    required_cols = ['filename', 'start', 'stop', 'label']
    for x in required_cols:
        assert x in df.columns.values, 'Column {0} missing from input table'.format(x)

    # check for labeling conflict
    if signal_labels is not None and len(backgr_labels) > 0:
        common_labels = [x for x in signal_labels if x in backgr_labels]
        assert len(common_labels) == 0, 'Duplication of labels in signal_labels and backgr_labels'

    # unique labels in input table
    labels = list(set(df['label'].values)) 

    # convert to str
    _backgr_labels = [str(x) for x in backgr_labels] 
    backgr_labels = _backgr_labels
    if signal_labels is not None:
        _signal_labels = [str(x) for x in signal_labels] 
        signal_labels = _signal_labels

    # separate out background labels, if any
    for x in backgr_labels:
        assert x in labels, 'label {0} not found in input table'.format(x)
    
    _labels = [x for x in labels if x not in backgr_labels]
    labels = _labels
    ...
    # separate out remaining labels
    remaining_labels = []
    for x in signal_labels:
        assert x in labels, 'label {0} not found in input table'.format(x)

    remaining_labels = [x for x in labels if x not in signal_labels]
    ...
    df['label'] = df['label'].apply(lambda x: labels_dict.get(x))

    return df


def create_annotation_table(path, seg_len, columns=None, balancing_method=None, labels=None, backgr_labels=0,\
    rndm_backgr=None):
